# jarvis.py – FRIDAY Agent v5.1
# Run: streamlit run jarvis.py
# ──────────────────────────────────────────────────────────────────────────────
# Required packages:
#   pip install streamlit psutil requests pyttsx3
#   pip install SpeechRecognition ollama PyPDF2 python-docx reportlab
# version..v5.1  [FIXED: Removed st_autorefresh — replaced with @st.fragment]
#               [FIXED: Live Status updates every 1s WITHOUT tab redirect]
#               [FIXED: Nested expander bug — replaced with st.code]
#               [ADDED: Document Analyzer History tab]
#               [ADDED: Document Analyzer Progress tracking]
...
def system_monitor_thread():
    ...
    while monitor_running:
        try:
            ...
            pass
        # ── FIXED: 1 second interval for real-time updates ──
        time.sleep(1)
...
def agentic_process(query, uploaded_content=""):
    ...
    version_action = (
        ...
        "- V4.0: Autorefresh-kill bug fixed.\n"
        "- V4.2: All tab redirect bugs fixed — inline processing.\n"
        "- V5.0: Directive card UI, History tabs for all features.\n"
        "- V5.1: FIXED st_autorefresh → @st.fragment live status. Doc Analyzer history added.\n"
        "- V5.2: FIXED nested expander bug. Real-time 1s updates."
    )
...
# ==================================================
# DIRECTIVE CARD HTML HELPER
# ==================================================
def render_directive_card(t: dict, compact: bool = False) -> str:
    tid     = t.get("id", "---")
    goal    = t.get("goal", "No goal set")
    status  = t.get("status", "pending")
    created = str(t.get("created", ""))[:16]

    status_color = "#00e676" if status == "pending" else "#ff9500" if status == "active" else "#888"
    status_bg    = "#0d2d1a" if status == "pending" else "#2d1a0d" if status == "active" else "#222"
    ...
    if compact:
        ...
        ">
            ...
            <div style="color:#e0e0e0;font-size:13px;line-height:1.5;margin-bottom:4px;">
                {goal[:60]}{"..." if len(goal)>60 else ""}
            </div>
            <div style="color:#666;font-size:11px;">🕐 {created}</div>
        </div>
        """
    else:
        return f"""
        ...
        ">
            ...
            <div style="color:#ffffff;font-size:15px;line-height:1.6;margin-bottom:6px;font-weight:500;">
                {goal}
            </div>
            <div style="color:#666;font-size:12px;">🕐 Created: {created}</div>
        </div>
        """


# ==================================================
# ★ LIVE STATUS PANEL — @st.fragment
#   KEY FIX: run_every=1 for real-time 1-second updates
#   Only this component reruns every second.
#   Tabs, inputs, and all other widgets stay stable. ✅
# ==================================================
@st.fragment(run_every=1)
def live_status_panel():
    """Isolated fragment: reruns every 1s WITHOUT touching tabs or other widgets."""
    # Drain the monitor queue inside the fragment
    while not monitor_queue.empty():
        try:
            typ, val = monitor_queue.get_nowait()
            if typ == "stats":
    ...
    st.markdown(f"""
        <div class="live-status-panel">
          <h2>⚡ LIVE STATUS</h2>
          <h3>Real-time System Health &amp; Runtime Metrics — Auto-updates every 1s</h3>
...
def main():
    ...
    _defaults = {
        ...
        "doc_file_bytes":        None,
        "doc_file_name":         "",
        # ── Document Analyzer history list ──
        "doc_history":           [],   # [{filename, timestamp, file_size, chars, analysis, raw_preview}]
        "study_plan":            None,
        "voice_transcript":      "",
        "formatted_notes":       "",
        "exam_answer":           "",
    ...
    .hist-card-title { color:#ff9500;font-weight:bold;font-size:14px;margin-bottom:4px; }
    .hist-card-meta  { color:#888;font-size:12px;margin-bottom:6px; }
    .hist-card-body  { color:#ccc;font-size:13px;line-height:1.5; }
    .hist-empty {
        background:#141922;border-radius:9px;padding:20px;
        text-align:center;color:#666;font-size:14px;
        border:1px dashed #333;margin:10px 0;
    }
    /* Doc progress mini bar */
    .doc-prog-bar {
        background:#222; border-radius:4px; height:8px; margin:4px 0; overflow:hidden;
    }
    .doc-prog-fill {
        background:linear-gradient(90deg,#ff9500,#ffb347); height:8px; border-radius:4px;
    }
    /* Raw text preview box (replaces nested expander) */
    .raw-text-box {
        background:#0d1117;
    ...
    # ── Drain monitor queue at top level for marquee stats ────────────────
    while not monitor_queue.empty():
        try:
            typ, val = monitor_queue.get_nowait()
            if typ == "stats":
    ...
    # ── MARQUEE (no autorefresh — just shows last known stats) ────────────
    ...
    # ══════════════════════════════════════════════════════════════════════
    if st.session_state.mode == "Agent":
        st.markdown("<h1>FRIDAY – Grandmaster Agent Protocol v5.2</h1>", unsafe_allow_html=True)
        ...
        with st.sidebar:
            ...
            if st.button("📋 Reveal Pending Directives"):
                st.session_state.show_directives = not st.session_state.get("show_directives", False)

            if st.session_state.get("show_directives", False):
                tasks = memory.get_recent_tasks()
                if tasks:
                    for t in tasks:
                        st.markdown(render_directive_card(t, compact=True), unsafe_allow_html=True)
                else:
                    st.markdown(
                        '<div class="hist-empty">Directive Queue Empty</div>',
                        unsafe_allow_html=True
                    )
        ...
        with tab1:
            ...
            # ── Live Status Panel — @st.fragment handles its own 1s refresh ──
            live_status_panel()
        ...
        # ── TAB 3 : DIRECTIVE QUEUE ───────────────────────────────────────
        with tab3:
            st.subheader("Strategic Directive Queue")
            tasks = memory.get_recent_tasks()
            if tasks:
                for t in tasks:
                    st.markdown(render_directive_card(t, compact=False), unsafe_allow_html=True)
            else:
                st.markdown(
                    '<div class="hist-empty">📭 Directive Queue Empty — Deploy \'Add Directive: Objective\'</div>',
                    unsafe_allow_html=True
                )
            st.markdown("---")
        ...
        with tab4:
            ...
            # ── Three sub-tabs: Analyze | Progress | History ──────────────
            doc_sub1, doc_sub2, doc_sub3 = st.tabs([
                "🔍 Analyze Document", "📊 Analysis Progress", "📁 Analysis History"
            ])

            # ── DOC SUB-TAB 1: Analyze ────────────────────────────────────
            with doc_sub1:
                ...
                with clear_col:
                    clear_doc_btn = st.button("🗑️ Clear", key="clear_doc_btn", use_container_width=True)
                    ...
                    if not st.session_state.doc_file_bytes:
                        ...
                        if extracted and extracted.strip():
                            ...
                            hist_entry = {
                                ...
                                "chars":       len(extracted),
                                "analysis":    analysis,
                                "raw_preview": extracted[:400]
                            }
                            st.session_state.doc_history.append(hist_entry)
                ...
                if st.session_state.doc_analysis:
                    ...
                    # ── FIXED: replaced nested st.expander with direct display ──
                    st.markdown("**📃 View Extracted Raw Text:**")
                    raw_preview = st.session_state.doc_raw_text[:4000]
                    st.markdown(
                        f'<div class="raw-text-box">{raw_preview}</div>',
                        unsafe_allow_html=True
                    )

            # ── DOC SUB-TAB 2: Progress ───────────────────────────────────
            with doc_sub2:
                st.markdown("### 📊 Document Analysis Progress Tracker")
                doc_hist = st.session_state.doc_history

                if not doc_hist:
                    st.markdown(
                        '<div class="hist-empty">📭 No documents analyzed yet.<br>'
                        'Analyze documents in the first tab — progress appears here!</div>',
                        unsafe_allow_html=True
                    )
                else:
                    total_docs    = len(doc_hist)
                    total_chars   = sum(e.get("chars", 0) for e in doc_hist)
                    total_kb      = sum(e.get("file_size_kb", 0) for e in doc_hist)
                    with_deadline = sum(
                        1 for e in doc_hist
                        if e.get("analysis", {}).get("deadline", "Not mentioned").lower() not in
                           ["not mentioned", "n/a", ""]
                    )
                    with_action   = sum(
                        1 for e in doc_hist
                        if e.get("analysis", {}).get("action", "No action required").lower() not in
                    ...
                    with c1:
                        ...
                        <div class="metric-card" style="text-align:center;">
                            <div style="font-size:32px;font-weight:bold;color:#ff9500;">{total_docs}</div>
                            <div style="color:#aaa;font-size:13px;">Documents Analyzed</div>
                        </div>""", unsafe_allow_html=True)
                    with c2:
                        st.markdown(f"""
                        <div class="metric-card" style="text-align:center;">
                            <div style="font-size:32px;font-weight:bold;color:#00e676;">{total_kb:.1f} KB</div>
                            <div style="color:#aaa;font-size:13px;">Total Data Processed</div>
                        </div>""", unsafe_allow_html=True)
                    with c3:
                        st.markdown(f"""
                        <div class="metric-card" style="text-align:center;">
                            <div style="font-size:32px;font-weight:bold;color:#64b5f6;">{with_action}</div>
                            <div style="color:#aaa;font-size:13px;">Required Action</div>
                        </div>""", unsafe_allow_html=True)
                    with c4:
                        st.markdown(f"""
                        <div class="metric-card" style="text-align:center;">
                            <div style="font-size:32px;font-weight:bold;color:#ff5252;">{with_deadline}</div>
                            <div style="color:#aaa;font-size:13px;">Had Deadlines</div>
                        </div>""", unsafe_allow_html=True)

                    st.markdown("---")
                    st.markdown("#### 📈 Per-Document Breakdown")

                    max_chars = max((e.get("chars", 1) for e in doc_hist), default=1)
                    for i, entry in enumerate(reversed(doc_hist), 1):
                        a           = entry.get("analysis", {})
                        chars       = entry.get("chars", 0)
                        pct         = min(int((chars / max_chars) * 100), 100)
                        has_deadline = a.get("deadline", "Not mentioned").lower() not in ["not mentioned", "n/a", ""]
                        has_action   = a.get("action", "No action required").lower() not in ["no action required", "n/a", ""]
                        deadline_badge = (
                            f'<span style="background:#3a1a1a;color:#ff5252;padding:2px 8px;border-radius:10px;'
                            f'font-size:11px;border:1px solid #ff5252;">⏰ Deadline</span>'
                            if has_deadline else
                            f'<span style="background:#1a3a1a;color:#888;padding:2px 8px;border-radius:10px;font-size:11px;">No Deadline</span>'
                        )
                        action_badge = (
                            f'<span style="background:#1a2a3a;color:#64b5f6;padding:2px 8px;border-radius:10px;'
                            f'font-size:11px;border:1px solid #64b5f6;">✅ Action Needed</span>'
                            if has_action else
                            f'<span style="background:#1a3a1a;color:#888;padding:2px 8px;border-radius:10px;font-size:11px;">No Action</span>'
                        )
                        st.markdown(f"""
                        <div style="background:#141922;border-left:3px solid #ff9500;border-radius:9px;
                                    padding:12px 15px;margin:7px 0;">
                            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px;">
                                <span style="color:#ff9500;font-weight:bold;font-size:14px;">
                                    #{i} &nbsp; {entry.get('filename','Unknown')}
                                </span>
                                <span style="color:#666;font-size:12px;">🕐 {entry.get('timestamp','')}</span>
                            </div>
                            <div style="display:flex;gap:6px;margin:6px 0;flex-wrap:wrap;">
                                {deadline_badge} {action_badge}
                                <span style="background:#1a1a2a;color:#aaa;padding:2px 8px;border-radius:10px;font-size:11px;">
                                    {entry.get('file_size_kb',0)} KB &nbsp;|&nbsp; {chars:,} chars
                                </span>
                            </div>
                            <div style="color:#888;font-size:12px;margin-bottom:4px;">Text Extraction Progress:</div>
                            <div class="doc-prog-bar">
                                <div class="doc-prog-fill" style="width:{pct}%;"></div>
                            </div>
                            <div style="color:#aaa;font-size:12px;margin-top:4px;">
                                Topic: <span style="color:#fff;">{a.get('topic','N/A')[:80]}</span>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    st.markdown("---")
                    st.download_button(
                        "💾 Export All Analysis Data (JSON)",
                        data=json.dumps(st.session_state.doc_history, indent=2),
                        file_name=f"doc_analysis_history_{datetime.now().strftime('%Y%m%d_%H%M')}.json",
                        mime="application/json"
                    )

            # ── DOC SUB-TAB 3: History ────────────────────────────────────
            with doc_sub3:
                st.markdown("### 📁 Document Analysis History")
                doc_hist = st.session_state.doc_history

                if not doc_hist:
                    st.markdown(
                        '<div class="hist-empty">📭 No documents analyzed yet.<br>'
                        'Analyze a document in the first tab — it will appear here!</div>',
                        unsafe_allow_html=True
                    )
                else:
                    st.caption(f"Total documents analyzed this session: **{len(doc_hist)}**")

                    for i, entry in enumerate(reversed(doc_hist)):
                        idx = len(doc_hist) - 1 - i
                        a   = entry.get("analysis", {})
                        has_deadline = a.get("deadline", "Not mentioned").lower() not in ["not mentioned", "n/a", ""]

                        with st.expander(
                            f"📄 {entry.get('filename', 'Unknown')}  |  {entry.get('timestamp','')[:16]}  |  {entry.get('file_size_kb',0)} KB"
                        ):
                            col_l, col_r = st.columns([3, 1])
                            with col_l:
                                st.markdown(f"""
                                <div class="hist-card">
                                    <div class="hist-card-title">📄 {entry.get('filename','Unknown')}</div>
                                    <div class="hist-card-meta">
                                        🕐 {entry.get('timestamp','')} &nbsp;|&nbsp;
                                        📦 {entry.get('file_size_kb',0)} KB &nbsp;|&nbsp;
                                        🔤 {entry.get('chars',0):,} chars extracted
                                    </div>
                                    <div class="hist-card-body">
                                        <b>Topic:</b> {a.get('topic','N/A')}<br>
                                        <b>Action:</b> {a.get('action','N/A')}<br>
                                        <b>Deadline:</b>
                                        <span style="color:{'#ff5252' if has_deadline else '#888'};">
                                            {a.get('deadline','Not mentioned')}
                                        </span>
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
                            with col_r:
                                ...
                                <div style="text-align:center;padding:12px;">
                                    <div style="font-size:28px;">
                                        {'⚠️' if has_deadline else '✅'}
                                    </div>
                                    <div style="color:#888;font-size:11px;margin-top:4px;">
                                        {'HAS DEADLINE' if has_deadline else 'NO DEADLINE'}
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            <div class="feature-card" style="margin-top:8px;">
                                ...
                                <div class="analysis-section">
                                    <div class="analysis-label">✅ Action Required</div>
                                    <div class="analysis-value">{a.get('action','N/A')}</div>
                                </div>
                                <div class="analysis-section">
                                    <div class="analysis-label">⏰ Deadline</div>
                                    <div class="analysis-value" style="color:{'#ff5252' if has_deadline else '#fff'};">
                                        {a.get('deadline','Not mentioned')}
                                    </div>
                                </div>
                                <div class="analysis-section">
                                    <div class="analysis-label">⚠️ Consequence</div>
                                    <div class="analysis-value">{a.get('consequence','N/A')}</div>
                                </div>
                            </div>
                            """, unsafe_allow_html=True)

                            # ── FIXED: replaced nested st.expander with direct styled div ──
                            st.markdown("**📃 Raw Text Preview:**")
                            raw_prev = entry.get("raw_preview", "")[:400]
                            st.markdown(
                                f'<div class="raw-text-box">{raw_prev}</div>',
                                unsafe_allow_html=True
                            )
                            ...
                            st.download_button(
                                ...
                                file_name=f"analysis_{entry.get('filename','doc')}_{entry.get('timestamp','')[:10]}.json",
                                mime="application/json",
                                key=f"dl_doc_{idx}"
                            )
        ...
        with tab5:
            ...
            planner_sub1, planner_sub2, planner_sub3 = st.tabs([
                "➕ Create New Plan", "📊 Track Progress", "📁 Plan History"
            ])
                    ...
                    if not subjects:
                        ...
                        with st.spinner("Building optimised timetable..."):
                            plan = generate_study_plan(
                                exam_name, exam_date_in.strftime("%Y-%m-%d"),
                                subjects, weak_areas, daily_h
                            )
                        if "error" in plan:
                            st.error(plan["error"])
                        else:
                            ...
                            st.success(
                                f"✅ Timetable created! **{plan['days_left']} days** until exam "
                                f"• **{len(plan['schedule'])} study days** planned"
                            )
            ...
            with planner_sub3:
                st.markdown("### 📁 All Study Plans — History")
                all_plans = st.session_state.study_planner_data.get("plans", [])
                if not all_plans:
                    st.markdown(
                        '<div class="hist-empty">📭 No plans created yet.<br>Start by creating a plan in the first tab!</div>',
                        unsafe_allow_html=True
                    )
                else:
                    st.caption(f"Total plans created: **{len(all_plans)}**")
                    for plan in reversed(all_plans):
                        prog     = get_study_progress(plan)
                        overall  = prog.get("overall", 0)
                        created  = str(plan.get("created",""))[:16]
                        exam_dt  = datetime.strptime(plan["exam_date"], "%Y-%m-%d").date()
                        days_l   = (exam_dt - date.today()).days
                        days_str = (f"⏳ {days_l}d left" if days_l > 0
                                    else "📅 Exam today!" if days_l == 0
                                    else f"✅ Done ({abs(days_l)}d ago)")
                        subj_list = ", ".join(plan.get("subjects", []))
                        weak_list = ", ".join(plan.get("weak_areas", [])) or "None"
                        pct_color = "#00e676" if overall >= 70 else "#ff9500" if overall >= 30 else "#ff5252"

                        with st.expander(
                            f"📚 {plan['exam_name']}  |  Exam: {plan['exam_date']}  |  Progress: {overall}%"
                        ):
                            col_a, col_b = st.columns([2, 1])
                            with col_a:
                                st.markdown(f"""
                                <div class="hist-card">
                                    <div class="hist-card-title">📘 {plan['exam_name']}</div>
                                    <div class="hist-card-meta">
                                        🗓️ Created: {created} &nbsp;|&nbsp; 📅 Exam: {plan['exam_date']} &nbsp;|&nbsp; {days_str}
                                    </div>
                                    <div class="hist-card-body">
                                        <b>Subjects:</b> {subj_list}<br>
                                        <b>Weak Areas:</b> {weak_list}<br>
                                        <b>Daily Hours:</b> {plan.get('daily_hours', '?')}h &nbsp;|&nbsp;
                                        <b>Study Days:</b> {len(plan.get('schedule', {}))}
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
                            with col_b:
                                ...
                                <div style="text-align:center;padding:16px;">
                                    <div style="font-size:36px;font-weight:bold;color:{pct_color};">{overall}%</div>
                                    <div style="color:#888;font-size:12px;margin-top:4px;">Progress</div>
                                    <div class="progress-container" style="margin-top:8px;">
                                        <div class="progress-fill" style="width:{overall}%;"></div>
                                    </div>
                                    <div style="color:#aaa;font-size:12px;margin-top:6px;">
                                        {prog.get('done_hours',0):.1f}h / {prog.get('total_hours',0):.1f}h
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)

                            st.markdown("**Subject Breakdown:**")
                            for subj, pct in prog.get("per_subject", {}).items():
                                badge = " 🔴" if subj in plan.get("weak_areas", []) else ""
                                st.markdown(f"""
                                <div style="margin:5px 0;">
                                    <span style="color:#ff9500;font-size:13px;">{subj}{badge}</span>
                            ...
                            st.download_button(
                                "💾 Download Plan (JSON)",
                                data=json.dumps(plan, indent=2),
                                file_name=f"plan_{plan['id']}.json",
                                mime="application/json",
        ...
        with tab6:
            ...
            vs_tab1, vs_tab2 = st.tabs(["🎙️ Record & Format", "📋 Session History"])
                ...
                with col_rec:
                    ...
                    if st.session_state.voice_transcript and not st.session_state.voice_transcript.startswith("❌"):
                        ...
                        edited_t = st.text_area(
                            ...
                            height=130, key="edit_raw_transcript"
                        )
                        ...
                        if st.button("✨ Format Notes with AI", key="format_notes_btn"):
                            ...
                            with st.spinner("AI is structuring your notes..."):
                                formatted = format_notes_with_ai(
                                    edited_t,
                                    note_title or "Voice Notes"
                                )
                            st.session_state.formatted_notes  = formatted
                            st.session_state.voice_transcript = edited_t
                            st.session_state.ai_processing    = False
                            ...
                            if formatted and not formatted.startswith("❌"):
                                hist_entry = {
                                    ...
                                    "content":    formatted,
                                    "transcript": edited_t[:300]
                                }
                                st.session_state.voice_history.append(hist_entry)
            ...
            with vs_tab2:
                st.markdown("### 📋 Voice Note Session History")
                hist = st.session_state.voice_history
                if not hist:
                    st.markdown(
                        '<div class="hist-empty">📭 No sessions yet.<br>Format notes in the Record & Format tab to see history here!</div>',
                        unsafe_allow_html=True
                    )
                else:
                    st.caption(f"Total sessions this run: **{len(hist)}**")
                    for i, entry in enumerate(reversed(hist)):
                        idx = len(hist) - 1 - i
                        with st.expander(f"🎙️ {entry['title']}  |  {entry['timestamp']}"):
                            st.markdown(f"""
                            <div class="hist-card">
                                <div class="hist-card-title">📝 {entry['title']}</div>
                                <div class="hist-card-meta">🕐 {entry['timestamp']}</div>
                                <div class="hist-card-body"><b>Preview:</b> {entry['preview']}...</div>
                            </div>
                            """, unsafe_allow_html=True)
                            st.markdown("**Full Formatted Notes:**")
                            st.markdown(entry['content'])
                            if st.button(f"🔄 Restore This Session", key=f"restore_vs_{idx}"):
                                st.session_state.formatted_notes = entry['content']
                                st.success("Session restored to preview panel!")
                            safe = re.sub(r'[^\w]', '_', entry['title'][:30])
                            st.download_button(
                                "📥 Download as TXT",
                                data=entry['content'],
                                file_name=f"{safe}_{entry['timestamp'][:10]}.txt",
                                mime="text/plain",
                                key=f"dl_vs_{idx}"
                            )
        ...
        with tab7:
            ...
            ea_tab1, ea_tab2 = st.tabs(["✏️ Generate Answer", "📋 Answer History"])
            ...
            with ea_tab1:
                ...
                if st.button("🎯 Generate Exam Answer", key="gen_exam_btn"):
                    if not exam_question.strip():
                        ...
                        st.session_state.ai_processing = True
                        wait_msg = ("10–20 sec" if mark_type == "2 Marks"
                                    else "20–40 sec" if mark_type == "5 Marks"
                                    else "40–90 sec")
                        st.markdown('<div class="processing-banner">🧠 WRITING EXAM ANSWER — PLEASE WAIT</div>', unsafe_allow_html=True)
                        with st.spinner(f"🧠 Writing {mark_type} answer... ({wait_msg})"):
                            answer = generate_exam_answer(exam_question, mark_type, subject_hint)
                        st.session_state.exam_answer   = answer
                        st.session_state.ai_processing = False
                        voice.speak(f"{mark_type} exam answer generated.")
                ...
                if st.session_state.exam_answer:
                    st.markdown("---")
                    mark_color = {"2 Marks": "#4CAF50", "5 Marks": "#2196F3", "10 Marks": "#ff9500"}.get(mark_type, "#ff9500")
                    st.markdown(
                        ...
                        f"margin-bottom:10px;'>{mark_type} Answer</div>",
                        unsafe_allow_html=True
                    )
                    with st.container():
                        st.markdown(
                    ...
                    with x1:
                        if reportlab_available:
                            ...
                            pdf_b  = export_notes_to_pdf(f"Exam Answer – {mark_type}", full_c)
                            if pdf_b:
                    ...
                    with x2:
                        if docx_available:
                            ...
                            docx_b = export_notes_to_docx(f"Exam Answer – {mark_type}", full_c)
                            if docx_b:
            ...
            with ea_tab2:
                ...
                ea_hist = st.session_state.exam_history
                if not ea_hist:
                    st.markdown(
                        '<div class="hist-empty">📭 No answers generated yet.<br>Generate answers in the first tab — they appear here automatically!</div>',
                        unsafe_allow_html=True
                    )
                else:
                    st.caption(f"Total answers this run: **{len(ea_hist)}**")
                    mark_color_map = {"2 Marks": "#4CAF50", "5 Marks": "#2196F3", "10 Marks": "#ff9500"}
                    for i, entry in enumerate(reversed(ea_hist)):
                        idx = len(ea_hist) - 1 - i
                        mc  = mark_color_map.get(entry['mark_type'], '#ff9500')
                        with st.expander(
                            f"✏️ {entry['question'][:55]}...  |  {entry['mark_type']}  |  {entry['timestamp']}"
                        ):
                            st.markdown(f"""
                            <div class="hist-card">
                                <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                                    <span style="background:{mc};color:#000;padding:2px 10px;
                                        border-radius:14px;font-size:12px;font-weight:bold;">
                                        {entry['mark_type']}
                                    </span>
                                    <span style="background:#1a2a3a;color:#64b5f6;padding:2px 10px;
                                        border-radius:14px;font-size:12px;border:1px solid #64b5f6;">
                                        {entry['subject']}
                                    </span>
                                    <span class="hist-card-meta">🕐 {entry['timestamp']}</span>
                                </div>
                                <div class="hist-card-title">Q: {entry['question']}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            st.markdown("**Answer:**")
                            st.markdown(
                                f'<div class="exam-answer">{entry["answer"].replace(chr(10), "<br>")}</div>',
                                unsafe_allow_html=True
                            )
                            if st.button(f"🔄 Restore This Answer", key=f"restore_ea_{idx}"):
                                st.session_state.exam_answer = entry['answer']
                                st.success("Answer restored!")
                            q_s = re.sub(r'[^\w]', '_', entry['question'][:30])
                            st.download_button(
                                "📥 Download as TXT",
                                data=f"Q: {entry['question']}\n\n{entry['answer']}",
                                file_name=f"exam_{q_s}.txt",
                                mime="text/plain",
                                key=f"dl_ea_{idx}"
                            )
        ...
        with tab8:
            ...
            ct_main, ct_hist = st.tabs(["🚀 Tutor Session", "📋 Session History"])
            ...
            with ct_main:
                ...
                with ct_col2:
                    ...
                    ct_action = st.radio("🎯 What do you want?", list(CODING_ACTIONS.keys()), key="ct_action")
                    st.markdown("---")
                    action_descriptions = {
                ...
                if st.button("🚀 Run Coding Tutor", key="run_ct_btn"):
                    if not ct_code.strip():
                        ...
                        with st.spinner(f"🧠 FRIDAY is processing your {ct_lang} code..."):
                            result = run_coding_tutor(ct_action, ct_code, ct_lang)
                        st.session_state.coding_result  = result
                        st.session_state.ai_processing  = False
                        voice.speak(f"Coding tutor response ready for {ct_lang}.")

                        if result and not result.startswith("❌"):
                            st.session_state.coding_history.append({
                                "language":     ct_lang,
                                "action":       ct_action,
                                "code_snippet": ct_code[:200],
                                "result":       result,
                                "timestamp":    datetime.now().strftime("%Y-%m-%d %H:%M")
                            })
                ...
                if st.session_state.coding_result:
                    ...
                    st.markdown(
                        ...
                        f"margin-bottom:10px;'>{ct_action} · {ct_lang}</div>",
                        unsafe_allow_html=True
                    )
                    ...
                    with exp1:
                        if reportlab_available:
                            pdf_b = export_notes_to_pdf(f"Coding Tutor – {ct_action}", full_export)
                            if pdf_b:
                    ...
                    with exp2:
                        if docx_available:
                            docx_b = export_notes_to_docx(f"Coding Tutor – {ct_action}", full_export)
                            if docx_b:
            ...
            with ct_hist:
                st.markdown("### 📋 Coding Session History")
                ct_h = st.session_state.coding_history
                if not ct_h:
                    st.markdown(
                        '<div class="hist-empty">📭 No sessions yet.<br>Run the Coding Tutor and results appear here automatically!</div>',
                        unsafe_allow_html=True
                    )
                else:
                    st.caption(f"Total sessions this run: **{len(ct_h)}**")
                    lang_color_map = {
                        "Python": "#3776ab", "C": "#a8b9cc", "C++": "#00599c",
                        "Java": "#f89820", "JavaScript": "#f7df1e", "SQL": "#e34c26",
                        "HTML/CSS": "#e44d26"
                    }
                    for i, entry in enumerate(reversed(ct_h)):
                        idx = len(ct_h) - 1 - i
                        lc  = lang_color_map.get(entry['language'], '#ff9500')
                        with st.expander(
                            f"💻 {entry['action']}  |  {entry['language']}  |  {entry['timestamp']}"
                        ):
                            st.markdown(f"""
                            <div class="hist-card">
                                <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                                    <span style="background:{lc};color:#000;padding:2px 10px;
                                        border-radius:14px;font-size:12px;font-weight:bold;">
                                        {entry['language']}
                                    </span>
                                    <span style="background:#1e1e1e;color:#ff9500;padding:2px 10px;
                                        border-radius:14px;font-size:12px;border:1px solid #ff9500;">
                                        {entry['action']}
                                    </span>
                                    <span class="hist-card-meta">🕐 {entry['timestamp']}</span>
                                </div>
                                <div class="hist-card-body">
                                    <b>Code/Input:</b><br>
                                    <code style="color:#a8ff78;font-size:12px;">{entry['code_snippet'][:150]}{"..." if len(entry['code_snippet'])>150 else ""}</code>
                                </div>
                            </div>
                            """, unsafe_allow_html=True)
                            st.markdown("**FRIDAY's Response:**")
                            st.markdown(entry['result'])
                            if st.button(f"🔄 Restore This Session", key=f"restore_ct_{idx}"):
                                st.session_state.coding_result = entry['result']
                                st.success("Session restored!")
                            cs = re.sub(r'[^\w]', '_', entry['code_snippet'][:25])
                            full_exp = (
            ...
            with iv_sub1:
                if st.session_state.interview_complete:
                    ...
                    if st.button("🚀 Start Mock Interview", key="start_iv_btn"):
                        ...
                        with st.spinner(f"Generating {iv_num_q} {iv_domain} questions..."):
                            questions = generate_interview_questions(iv_domain, iv_num_q)
                        if questions:
                            session_obj = {
                                "id":      f"IV{int(time.time())}",
                                "domain":  iv_domain,
                                "mode":    iv_mode,
                                "num_q":   len(questions),
                            ...
                            st.session_state.interview_q_idx    = 0
                            st.session_state.interview_answers   = []
                            st.session_state.interview_domain    = iv_domain
                            st.session_state.interview_active    = True
                            st.session_state.interview_complete  = False
                            st.session_state.interview_session   = session_obj
                            st.session_state["iv_answer_mode"]   = iv_mode
                            st.session_state.ai_processing       = False
            ...
            with iv_sub2:
                if not st.session_state.interview_active:
                    ...
                    if st.session_state.get('interview_eval_result'):
                        ...
                        st.markdown(f"**Q{eval_r['q_idx']+1} Feedback:**")
                        ...
                        if submit_btn:
                            if not candidate_answer.strip():
                                ...
                                with st.spinner("🧠 AI is evaluating your answer..."):
                                    feedback = evaluate_interview_answer(current_q, candidate_answer, domain)
                                score = feedback.get("score", 5)
                                grade = feedback.get("grade", "Average")
                                voice.speak(f"Question {q_idx+1} score: {score} out of 10. {grade}.")
                    ...
                    for entry in reversed(history[-10:]):
                        ...
                        with st.expander(f"🗓️ {sess.get('started','')[:16]}  |  {sess.get('domain','')}  |  Score: {avg_sc}/10"):
                            ...
                            st.download_button(
                                ...
                                data=json.dumps(entry, indent=2),
                                file_name=f"interview_{sess.get('id','')}.json",
                                mime="application/json",
