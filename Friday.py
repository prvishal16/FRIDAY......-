...
# ==================================================
def ollama_stream_response(prompt, model=OLLAMA_MODEL):
    if not ollama:
        yield "❌ Error: ollama python library is not installed.\n"
        yield "Please run: pip install ollama\n"
        return
    ...
    try:
        ...
        yield "Make sure Ollama is running on http://127.0.0.1:11434\n"
...
def ollama_chat_http(prompt, model=OLLAMA_MODEL):
    try:
        ...
        print("DEBUG: Ollama raw status:", response.status_code)
        print("DEBUG: Ollama raw text:", response.text)
        response.raise_for_status()
        data = response.json()
        reply = data.get("message", {}).get("content", "").strip()
        print("DEBUG: Final extracted reply:", reply)
        if not reply:
...
def main():
    ...
    # Show marquee + enable autorefresh ONLY in Agent mode
    if st.session_state.mode == "Agent":
        ...
        # Turn ON autorefresh in Agent mode (starts immediately when app loads)
    ...
    # Command Mode – NO autorefresh here
    else:
        # We do NOT call st_autorefresh() in Command mode → streaming won't be interrupted
        ...
        if "pending_prompt" in st.session_state:
            ...
            with st.chat_message("assistant"):
                ...
                placeholder.markdown(
                    "Core Intelligence Processing<span class='blinking-cursor'></span>",
                    unsafe_allow_html=True
                )
                ...
                for token in ollama_stream_response(current_prompt):
                    full_response += token
                    voice_buffer += token

                    placeholder.markdown(
                        f"{full_response}<span class='blinking-cursor'></span>",
                        unsafe_allow_html=True
                    )
