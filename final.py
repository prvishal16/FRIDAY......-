...
PASSWORD      = "1234"
# ── phi3:mini  = 3.8B params = ~2.3 GB RAM  (fits in 7 GB easily)
# ── deepseek-coder:1.3b = 1.3B params = ~776 MB RAM  (very fast)
GENERAL_MODEL = "phi3:mini"
CODING_MODEL  = "deepseek-coder:1.3b"
OLLAMA_MODEL  = GENERAL_MODEL   # used in marquee / status display
USE_OLLAMA    = True

# token budget per feature  (prevents truncated output)
TOKENS_FAST     = 150    # agent quick answers
TOKENS_JSON     = 350    # document analysis JSON, interview questions
TOKENS_MEDIUM   = 400    # coding tutor, voice notes, interview eval
TOKENS_EXAM     = 700    # exam answers (need full structured response)
...
# ==================================================
# CACHE LAYER  – keyed on prompt + model + num_predict
# ==================================================
...
# ==================================================
# OLLAMA NON-STREAMING  (speed + variable token budget)
# ==================================================
...
# ==================================================
# LIVE SYSTEM MONITOR THREAD  (5s → low CPU)
# ==================================================
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  ★ FEATURE 1 – DOCUMENT UNDERSTANDING ENGINE  (FIXED OUTPUT)           ║
# ╚══════════════════════════════════════════════════════════════════════════╝
def analyze_document_with_ai(text: str) -> dict:
    """
    Returns a dict with keys: topic, summary, action, deadline, consequence.
    Uses TOKENS_JSON (350) so the full JSON always arrives.
    """
    if not text.strip():
        return {
    ...
    # ── Compact prompt → less tokens needed for input, more for output ──
    prompt = (
    ...
    try:
        ...
        # graceful fallback: pull key fields from raw text
        def extract(key):
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  ★ FEATURE 2 – SMART STUDY PLANNER  (100% OFFLINE, NO AI NEEDED)       ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  ★ FEATURE 4 – EXAM ANSWER WRITING ASSISTANT  (FIXED OUTPUT)           ║
# ╚══════════════════════════════════════════════════════════════════════════╝
MARK_CONFIGS = {
    "2 Marks": {
        "target_words": "50–80",
...
def generate_exam_answer(question: str, mark_type: str, subject_hint: str = "") -> str:
    ...
    # Direct call (not cached) so each new question generates fresh output
...
def agentic_process(query, uploaded_content=""):
    ...
    version_action = (
        ...
        "- V4.0: Document Analyzer FIXED + Coding Tutor + Mock Interview added."
    )
...
def main():
    ...
    if st.session_state.mode == "Agent":
        ...
        st_autorefresh(interval=2000, key="agent_refresh")
    ...
    if st.session_state.mode == "Agent":
                ...
                if st.session_state.voice_transcript and not st.session_state.voice_transcript.startswith("❌"):
                    ...
                    if st.button("✨ Format Notes with AI", key="format_notes_btn"):
                        with st.spinner("AI is structuring your notes..."):
                            formatted = format_notes_with_ai(edited_t, note_title or "Voice Notes")
        ...
        with tab7:
            ...
            if st.button("🎯 Generate Exam Answer", key="gen_exam_btn"):
                if not exam_question.strip():
                    ...
                    with st.spinner(f"🧠 Writing {mark_type} answer... (may take 10–30 sec for 10 marks)"):
        ...
        with tab8:
            ...
            if st.session_state.coding_result:
                ...
                full_export = f"Language: {ct_lang}\nAction: {ct_action}\n\n" \
            ...
            with iv_sub1:
                if st.session_state.interview_complete:
                    ...
                    with col_restart[0]:
                        if st.button("🔄 Start New Interview", key="restart_iv_btn"):
                            for k in ["interview_active","interview_complete","interview_questions",
                                ...
                                st.session_state[k] = False if "active" in k or "complete" in k else ([] if "questions" in k or "answers" in k else (0 if "idx" in k else None))
                            st.rerun()
                    ...
                    if st.button("🚀 Start Mock Interview", key="start_iv_btn"):
                        ...
                        if questions:
                            session_obj = {
                                "id": f"IV{int(time.time())}", "domain": iv_domain,
                                "mode": iv_mode, "num_q": len(questions), "started": str(datetime.now())
                            }
                    ...
                    if q_idx >= len(questions):
                        ...
                        if submit_btn:
                            if not candidate_answer.strip():
                                ...
                                gc    = ("score-excellent" if score >= 8 else "score-good" if score >= 6 else "score-average" if score >= 4 else "score-poor")
                                ...
                                st.session_state.interview_answers.append({
                                    "question": current_q, "answer": candidate_answer,
                                ...
                                if q_idx + 1 >= len(questions):
                                    st.success("✅ That was the last question! Check your final report in **Start Interview** tab.")
                                else:
                                    if st.button("▶️ Next Question", key=f"next_q_{q_idx}", use_container_width=True):
                                        st.rerun()

                        if skip_btn:
                            st.session_state.interview_answers.append({
                                "question": current_q, "answer": "[Skipped]",
                                "score": 0, "grade": "Skipped", "strengths": "",
                                "improvements": "Practice answering this type of question.",
                                "ideal_answer": "", "tips": "Don't skip in real interviews."
                            })
                            st.session_state.interview_q_idx = q_idx + 1
                            st.rerun()
